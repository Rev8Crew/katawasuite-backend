...
var vnds_interpreter = function () {
    this.no_cache = '?updated=' + Math.floor(Date.now() / 1000);
        ...
        function draw_bg(obj, df, background, effect_name, effect_speed, is_remove_sprites) {
            ...
            function set_background(_this, obj, background) {
                ...
                if (is_hex_color(background.file))
                    ...
                    obj.css(
                        {
                            'background-image': 'url(' + window.k.formatAssetUrl(_this.game.dir + '/background/' + background.file) + ')',
    ...
    this.setimg = function (params, command_name = 'SETIMG') {
        ...
        function draw_sprite(obj, df, sprite, effect_speed) {
            ...
            if (sprite.path === undefined) // Если ссылка на новый файл спрайта не передана, то такой спрайт уже существует - просто двигаем его
            {
                ...
                if (sprite.name === 'img_rika' && sprite.y === 'center') {
                    y = 55 - (50 * $sprite_name.height() / resolution.height) + '%';
                }
            ...
            } else // Если ссылка таки была передана, загружаем спрайт
            {
                ...
                $('#' + sprite.name)
                    .attr('src', window.k.formatAssetUrl(sprite.path))
                    .one('load', function () {
    ...
    this.relay = function (params, command_name = 'RELAY') {
        ...
        if (this.relay_interval !== undefined) {
            $relay.stop().fadeTo(effect_speed / 2, 0, function () {
                $(this).css(
                    {
                        'background-image': 'url(' + window.k.formatAssetUrl(_this.game.dir + '/foreground/' + escape(images_list[0])) + ')'
                    });
        ...
        this.relay_interval = setInterval(function () {
            $relay.stop().fadeTo(effect_speed / 2, 0, function () {
                $(this).css(
                    {
                        'background-image': 'url(' + window.k.formatAssetUrl(_this.game.dir + '/foreground/' + escape(images_list[i++])) + ')'
                    });
                $(this).fadeTo(effect_speed / 2, 1);
  ...
  ====================================================================================================*/
    ...
    this.sound = function (params, command_name = 'SOUND') {
        ...
        if ((params_list[params_index].toLowerCase().search(/mp3|ogg|aac|wma/) !== -1) || (params_list[params_index] === '~'))
            sound_file = params_list[params_index++].replace('.ogg', '.mp3');
            ...
            if (sound_file === undefined) {
                ...
                window.k.asset(filename)
                    ...
                    .done(function () {
                        ...
                        $sound.attr('src', window.k.formatAssetUrl(filename));
                        $sound.trigger('play');
                        $sound.prop('volume', sound_volume);
  ...
  ====================================================================================================*/
    ...
    this.music = function (params, command_name = 'MUSIC') {
        ...
        let params_index = 0;
        if ((params_list[params_index].toLowerCase().search(/mp3|ogg|aac|wma/) !== -1) || (params_list[params_index] === '~'))
            music_file = params_list[params_index++].replace('.ogg', '.mp3');
        let effect_speed = 0
            ...
            if (music_file === undefined) {
                ...
                window.k.asset(filename)
                    ...
                    .done(function () {
                        if (config.is_check)
                            return;
                        $music.attr('src', window.k.formatAssetUrl(filename));
    ...
    this.video = function (params, command_name = 'VIDEO') {
        ...
        window.k.asset(filename)
            ...
            .done(function () {
                if (config.is_check)
                    return df.resolve();
                $video.attr('src', window.k.formatAssetUrl(filename));
  ...
  ====================================================================================================*/
    ...
    this.jump = function (params) {
        ...
        let _this = this;
        let script_lines;
        window.k.asset(this.game.dir + '/script/' + this.game.script_name + this.no_cache)
            .done(function (data) {
...
//				stop_all_effects();
        ...
        if (slot === '~') {
            try {
                ...
                $.each(localStorage, function (key, value) {
                    if (key.indexOf(window.k.getDefaultSlotName(_this.game.short_name, slot)) !== -1)
        ...
        let save_game =
            {
                ...
                music: Object.assign({}, this.game.music),
                sound: Object.assign({}, this.game.sound),
                bg_color: $('#game_screen').css('background-color'),
                bg: Object.assign({}, this.game.background),
                relay: Object.assign({}, this.game.relay),
                sprites: Object.assign({}, this.game.sprites),
                animation: Object.assign({}, this.game.animation)
            };

        //save_game = JSON.stringify(save_game);
        try {
            window.k.addSaveToSlot(save_game, slot);

            if (config.is_autosave) {
                config.autosave_lines -= 1
            }

            const isAutoSave = config.autosave_lines === 0
            if (slot !== 0 || isAutoSave) {
  ...
  ====================================================================================================*/
    ...
    this.delete = function (slot) {
        if (config.log_level !== LOG_DISABLE) console.log('DELETE ' + slot);
        ...
        let key = window.k.getDefaultSlotName(this.game.short_name, slot)

        if (localStorage.getItem(key) === null) {
            show_warning('Слот [' + slot + '] уже пустой');
            return false;
        }
    ...
    [short] - название достижения из БД
    ====================================================================================================*/

    this.achieve = function ( achieve_short ) {
        if (config.log_level !== LOG_DISABLE) console.log('ACHIEVE ' + achieve_short);
        let df = $.Deferred();

        if (achieve_short === null) {
            df.resolve('Неизвестная переменная функции ACHIEVE: ' + params);
            return df.promise();
        }

        window.$k.completeAchieve(achieve_short).then( (data) => {
            if (data && data?.name) {
                show_info('Вы получили достижение: ' + data?.name + ' (' + data?.description + ')');
            }
        })
...
// Получение сохранения
    this.get_load = function (slot) {
        let load_game;
        try {
            load_game = JSON.parse(localStorage.getItem(window.k.getDefaultSlotName(this.game.short_name,slot)));
        } catch (e) {
            show_warning('Ошибка в локальном хранилище: ' + e.name);
            return false;
        }
